...
/*   Updated: 2022/03/15 19:00:22 by rotrojan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		void test_insert(_map &X, _map &Y)
		{
			...
			try
			{
				switch (std::rand() % 4)
				{
					...
					case (4):
					{
						_NAMESPACE::map<int, int> first;
						_NAMESPACE::pair<_NAMESPACE::map<int, int>::iterator, bool> p;

						std::cout << first.size() << std::endl;
						for (int i = 0; i < 500; ++i) {
							p = first.insert(_NAMESPACE::make_pair<int, int>(i, 500 - i));
							std::cout << p.first->first << " " << p.first->second << " " << p.second << std::endl;
						}
						std::cout << first.size() << std::endl;
						_NAMESPACE::map<int, int> second;
						_NAMESPACE::map<int, int>::iterator it;
						_NAMESPACE::map<int, int>::iterator _it;
						for (it = first.begin(); it != first.end(); it++) {
							_it = second.insert(first.begin(), _NAMESPACE::make_pair<int, int>(it->first * 2, it->second * 4));
						}
						test_print(first);
						test_print(second);
					}
